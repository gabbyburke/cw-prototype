...
class BigQueryManager:
    ...
    def __init__(self):
        ...
        self.magic_button_table = f"{self.project_id}.{self.dataset_id}.magic_button"
    ...
    def associate_person_to_case(self, case_id: str, person_id: str, role_in_case: str) -> bool:
        ...
        try:
            ...
            query_job = self.client.query(case_check_query, job_config=job_config)
            case_results = list(query_job.result())
            
            if case_results[0].count == 0:
                logger.error(f"Case {case_id} does not exist")
                return False
            
            # Verify the person exists
            person_check_query = f"""
            SELECT COUNT(*) as count
            FROM `{self.people_table}`
            ...
            query_job = self.client.query(person_check_query, job_config=job_config)
            person_results = list(query_job.result())
            
            if person_results[0].count == 0:
                logger.error(f"Person {person_id} does not exist")
                return False
            
            # Check if the association already exists
            check_query = f"""
            SELECT COUNT(*) as count
            FROM `{self.case_person_associations_table}`
            WHERE case_id = @case_id 
            ...
            if results[0].count > 0:
                logger.warning(f"Person {person_id} is already associated with case {case_id}")
                return False
            ...
            insert_query = f"""
            INSERT INTO `{self.case_person_associations_table}` 
            (association_id, case_id, person_id, role_in_case, start_date, case_association)
            VALUES (@association_id, @case_id, @person_id, @role_in_case, CURRENT_DATE(), @case_association)
            """
            
            job_config = bigquery.QueryJobConfig(
                query_parameters=[
                    bigquery.ScalarQueryParameter("association_id", "STRING", association_id),
                    bigquery.ScalarQueryParameter("case_id", "STRING", case_id),
                    bigquery.ScalarQueryParameter("person_id", "STRING", person_id),
                    bigquery.ScalarQueryParameter("role_in_case", "STRING", role_in_case),
                    bigquery.ScalarQueryParameter("case_association", "STRING", f"{case_id}_{person_id}")
                ]
            )
            ...
            logger.info(f"Successfully associated person {person_id} to case {case_id} with role {role_in_case}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to associate person {person_id} to case {case_id}: {str(e)}")
            return False
    
    def get_magic_button_data_by_incident_number(self, incident_number: str) -> Optional[Dict[str, Any]]:
        """Get magic button data for a given incident number."""
        query = f"""
        SELECT
            *
        FROM `{self.magic_button_table}`
        ...
        job_config = bigquery.QueryJobConfig(
            query_parameters=[
                bigquery.ScalarQueryParameter("incident_number", "STRING", str(incident_number))
            ]
        )
        ...
        try:
            ...
            if not results:
                logger.warning(f"No magic button data found for incident: {incident_number}")
                return None
            
            # Convert the BigQuery Row to a dictionary
            magic_data = dict(results[0])
            logger.info(f"Retrieved magic button data for incident: {incident_number}")
            return magic_data
            
        except Exception as e:
            logger.error(f"Failed to retrieve magic button data for incident {incident_number}: {str(e)}")
            raise GoogleCloudError(f"Query failed: {str(e)}")
    
    def get_all_magic_button_incidents(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Retrieved {len(incidents)} magic button incidents")
            return incidents
            
        except Exception as e:
            logger.error(f"Failed to retrieve magic button incidents: {str(e)}")
            raise GoogleCloudError(f"Query failed: {str(e)}")
