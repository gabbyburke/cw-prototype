...
data "archive_file" "input_catalog_tarball" {
  ...
  source_dir  = "${path.module}/ccwis-case-management/src/backend"
}
...
resource "google_pubsub_topic_iam_member" "gcs_pubsub_publisher" {
  for_each = {
    "new-source" = google_pubsub_topic.new_source.name
  }
  topic  = each.value
...
module "wait_for_input_catalog_build" {
  ...
  create_cmd_body       = "${google_cloudbuild_trigger.input_catalog_build_trigger.location} input-catalog ${local.input_catalog_image_url} 5"
